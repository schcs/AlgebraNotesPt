...
$$
\left<u,v\right>=\frac 12(\|u+v\|-\|u\|-\|v\|).
$$
:::

:::{#exm-quadform}
1. Seja $V$ espaço com produto interno $\left<-,-\right>$. Então a aplicação $Q(v)=\|v\|^2$ é uma forma quadrática. 
...
temos que $B$ é uma forma bilinear em $\R^n$. Ora, observe que 
$$
Q(v)=v A_Qv^t=B(v,v)
$$
...
:::{#exr-}
Seja $V$, $B$ e $X$ como na @def-quad-mat e sejam $u,v\in V$. Demonstre que 
$$
B(u,v)=u_X\cdot[B]_X\cdot v_X^t \quad \mbox{e}\quad 
Q(u)=u_X\cdot [Q]_X\cdot u_X^t
$$ 
...
$$
u_Y^t\cdot A\cdot v_Y=B_Q(u,v)=u_X^t\cdot [B]_X\cdot v_X =(P\cdot u_Y)^t\cdot [B]_X\cdot (P\cdot u_Y)=
...
que
$$
B(y_{p+q+k},w)=(y_{p+q+k})_Y D w_Y^t=(D_{p+q+k,p+q+k}(y_{p+q+k})_Y) w_Y^t=0.
$$
...
e considere 
$$
0=B(v,y_k)=v_YD(y_k)_Y^t=D_{k,k}\alpha_k.
$$

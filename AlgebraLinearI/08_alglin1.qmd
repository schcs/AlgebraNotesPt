...
$$p_{A}(t) = {\textrm{Det}}(A- tI)$$ é chamado de *polinômio característico*
de $A$.
:::
...
Seja $$A = \begin{pmatrix}
    ...
    0 & 1 & 1 
    \end{pmatrix}.$$ 
    Vamos calcular o polinômio caraterístico de $A$:
$$
\begin{aligned}
    p_A(t) & = {\textrm{Det}}(A-tI) \\ 
    & = \tn{Det}\begin{pmatrix}
    4-t & 2 & 0 \\ 
    -1& 1-t & 0 \\
    0& 1 & 1-t 
...
p_B(t) & = & {\textrm{Det}}(B - t I) \\
&= & {\textrm{Det}}(P^{-1}AP - t I) \\
&= & {\textrm{Det}}(P^{-1}AP - t P^{-1}P)\\
&= & {\textrm{Det}}(P^{-1}(A-t I)P) \\
&= & {\textrm{Det}}(P^{-1}){\textrm{Det}}(A-t I){\textrm{Det}}(P) \\
& = & {\textrm{Det}}(A - t I) \\
& = & p_A(t).
\end{array}$$ ◻
:::
...
-   O *determinante* de $T$ é
    $${\textrm{Det}}(T) := {\textrm{Det}}([T]^{B}_{B}).$$

-   O *polinômio característico* de $T$ é
    $$p_{T}(t) = {\textrm{Det}}(T- tI) = \tn{Det}([T]_B^B - tI).$$
:::
...
${\textrm{Det}}(T) = \tn{Det}([T]_B^B) = 6$. A matriz $[T]_B^B$ é a matriz
...
se $$p_T(\alpha) = {\textrm{Det}}(T-\alpha I) = 0.$$
:::

:::{.proof}
Seja $\alpha\in\R$ um autovalor de $T$. Então existe um vetor não nulo, $\ul{v}$, tal que $T(\ul{v}) = \alpha \ul{v}$, mas isso é a mesma
coisa de dizer que $(T-\alpha I)v = \ul{0}$; ou seja $v\in\ker(T-\alpha I)$, logo ${\textrm{Det}}(T-\alpha I)= 0$.

Supondo que $\alpha$ é raiz do polinômio caraterístico, então
${\textrm{Det}}(T-\alpha I)= 0$, ou seja, existe $\ul{v}$ não-nulo tal que
...
$S_{\lambda} = {\textrm{Ker}}(T-\lambda I)$. Em particular, o autoespaço
...
$$\begin{array}{ccc}
    \ul{x} \in S_{\lambda} & \Longleftrightarrow & T(\ul{x}) = \lambda \ul{x} \\
     ...
     &\Longleftrightarrow & \ul{x} \in {\textrm{Ker}}(T-\lambda I).
...
\begin{aligned}
                ...
                & \Longleftrightarrow x=-y \ {\textrm{e }} \ y=z,
    \end{aligned} 
$$
portanto
...
\end{aligned}
$$
 então a matriz de $T$ com respeito à base $B$ é

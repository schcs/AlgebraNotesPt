...
:::{#exr-fourier}
Escreva uma função `fourier(f)` em Python que dada uma função $f\in C^\infty[a,b]$, calcula a aproximação de Fourier para a função $f$ até grau $5$.A sua função deve verificar as seguintes computações.
```python
fourier(x)

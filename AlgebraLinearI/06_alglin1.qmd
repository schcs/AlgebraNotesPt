...
uma matriz $n\times m$, usando as bases canônicas de $\R^m, \R^n$. De
fato, exatamente a mesma coisa funciona com quaisquer espaços de
dimensão finita e quaisquer bases:

Sejam $V, W$ espaços vetoriais com bases
$$B = \{\ul{b_1}\,,\,\ldots\,,\,\ul{b_m}\}$$
$$C = \{\ul{c_1}\,,\,\ldots\,,\,\ul{c_n}\}$$ respetivamente. Qualquer
...
$[T]_B^C$ a matriz $n\times m$ de $T$ com respeito às bases $B,C$. Isto
...
Observe que a matriz $[T]_B^C$ depende das escolhas das bases $B$ e $C$.
...
2 \\ -1
\end{pmatrix}.$$ Segue que a matriz $[I]_B^C$ da transformação
identidade com respeito às bases $B,C$ é $$[I]_B^C = \begin{pmatrix}
I(1,0)_C & I(0,1)_C
\end{pmatrix} = \begin{pmatrix}
-1 & 2 \\ 1 & -1
\end{pmatrix}.$$
...
dada por $T\mapsto [T]_B^C$ é um isomorfismo de espaços vetoriais.
:::

:::{.proof}

penúltimo teorema das notas de Semana 4), observando que o mesmo
argumento funciona. ◻
:::
...
transformações lineares. Então $$[T]_C^D\cdot [S]_B^C = [TS]_B^D.$$
...
do caso de $\R^n$ e as bases canônicas funciona (é o último teorema das
notas de Semana 4). ◻
:::
...
respetivamente, a matriz da transformação é $$[T]_B^C.$$ Agora suponha
que temos mais duas bases $B', C'$ de $V$ e $W$. Qual é a matriz de $T$
com respeito às bases novas $B',C'$? É o produto
$$\boxed{[T]_{B'}^{C'} = [I_W]_{C}^{C'}\cdot[T]_B^C\cdot[I_V]_{B'}^B}$$
(em que $I_V, I_W$ são as TLs identidades de $V,W$) pois
$$\begin{aligned}
  [I_W]_{C}^{C'}[T]_B^C[I_V]_{B'}^B & = [I_W]_{C}^{C'}[T\circ I_V]_{B'}^C \\
                 & = [I_W\circ T\circ I_V]_{B'}^{C'} \\
                 & = [T]_{B'}^{C'}.\qquad \checkmark
\end{aligned}$$

:::{#def-}
Quando $B,C$ são bases finitas do espaço vetorial $V$, a matriz
$[I_V]_B^C$ se chama da *matriz de mudança de base de $B$ para $C$*.
:::
...
questão nos deu a matriz $A = [T]_E^F$ e queremos calcular $[T]_B^C$.
Pela conversa acima, temos $$[T]_B^C = [I]_F^C[T]_E^F[I]_B^E,$$ então
temos que calcular $[I]_B^E$ e $[I]_F^C$.
...
-   Os vetores de coordenadas de $(1,1), (2,1)$ com respeito à base
    canônica $E$ são obviamente $$(1,1)_E = \begin{pmatrix}
      ...
      2 \\ 1
      \end{pmatrix},$$ então $$[I]_B^E = \begin{pmatrix}
      (1,1)_E & (2,1)_E
      \end{pmatrix} = \begin{pmatrix}
      1 & 2 \\ 1 & 1
      \end{pmatrix}.$$
...
-   Temos \begin{aligned}
    ...
    \end{aligned} Então $$(1,0,0)_C = \begin{pmatrix}
      ...
      0 \\ 0 \\ 1 
      \end{pmatrix}$$ logo $$[I]_F^C = \begin{pmatrix}
...
\begin{align*}
  [T]_B^C & = [I]_F^C\cdot[T]_E^F\cdot[I]_B^E \\
      & = \begin{pmatrix}
  1 & 0 & 0 \\ -1 & 1 & 0 \\ 0 & -1 & 1
  \end{pmatrix}\cdot \begin{pmatrix}
1 & 2 \\
...
duas bases de $V$. Seja $P$ a matriz $n\times n$ $$P = [I]_B^C,$$ a
matriz da transformação identidade de $V$ com respeito às bases $B,C$.
Considere também a matriz $[I]_C^B$. Temos
$$[I]_C^B\cdot [I]_B^C = [I\circ I]_B^B = [I]_B^B = \begin{pmatrix}1 & 0 & \cdots & 0 \\
0 & 1 &    & 0 \\
\vdots & & \ddots & \vdots \\
0 & 0 & \cdots & 1 
\end{pmatrix},$$ e similarmente $[I]_B^C\cdot [I]_C^B = I_n$. Ou seja, a
matriz $[I]_C^B$ é a matriz inversa de $[I]_B^C$:
$$P = [I]_B^C\quad,\quad P^{-1} = [I]_C^B.$$ [Exercício]{.underline}:
...
1.  $T$ é um isomorfismo se, e somente se, a matriz $[T]_B^C$ é
    invertível.

2.  Caso $T$ for isomorfismo, então $$[T^{-1}]_C^B = ([T]_B^C)^{-1}.$$
...
Já sabemos que $$[T]_B^B = [I]_C^B\cdot [T]_C^C \cdot [I]_B^C.$$ Mas se
$[I]_B^C = P$, então $[I]_C^B = P^{-1}$, então
...
R: Vamos calcular $P = [I]_B^E$. Os vetores de coordenadas dos vetores
...
1 \\ 0 \\ 2
\end{pmatrix}.$$ Logo a matriz $P$ é $$P = [I]_B^E = \begin{pmatrix}
0 & 1 & 1 \\ -1 & 1 & 0 \\ 0 & 1 & 2
\end{pmatrix}.$$ Usando as técnicas de GAAL, podemos calcular também que
$$[I]_E^B = P^{-1} = \begin{pmatrix}
0 & 1 & 1 \\ -1 & 1 & 0 \\ 0 & 1 & 2
\end{pmatrix}^{-1} = \begin{pmatrix}
2 & -1 & -1 \\ 2 & 0 & -1 \\ -1 & 0 & 1
\end{pmatrix}.$$ Agora 
\begin{align*}
  [T]_B^B & = [I]_E^B\cdot [T]_E^E \cdot [I]_B^E \\
...
três *invariantes* de matrizes semelhantes (duas esta semana, e a última
semana que vem) -- isto é, números associados a uma matriz quadrada que

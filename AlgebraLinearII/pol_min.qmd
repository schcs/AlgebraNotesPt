...
Suponhamos que $B=\{b_1,\ldots,b_n\}$ é uma base de $V$ e seja $(a_{i,j})=[f]_B^B$.   Considere a matriz
$$
A = \begin{pmatrix}
...
\end{pmatrix}
$$
A matriz $A$ é um elemento de $M_{n\times n}(R)$. A matriz $A$ pode ser vista como o resultado de substituir $f$ na matriz 
$t\cdot I-[f]_B^B=t\cdot I-(a_{i,j})$. De fato, nós substituímos $f$ no lugar da incôgnita $t$, e 
...
1. primeiro calcular $\mbox{pcar}_f(t)$ e substituir $f$ no lugar de $t$;
2. primeiro substituir $f$ em $t\cdot I-(a_{i,j})$ como no parágrafo anterior e calcular o determinante da matriz obtida desta forma.

Deixamos para o leitor verificar que estas duas maneiras de calcular $\mbox{pcar}_f(f)$ dão o mesmo resultado. Mas isso implica que 
$\det A  =\mbox{pcar}_f(f)$. Pondo $b=(b_1,\ldots,b_n)^t$ (vetor coluna), temos que $Ab=0$. Por outro lado, por @thm-cramer,
$$

...
$(u\otimes v)\otimes w\mapsto v\otimes (u\otimes w)$ ($u\in U$, $v\in V$ e $w\in W$) 
pode ser estendida unicamente para um isomorfismo bem definido $(U\otimes V)\otimes W\to V\otimes (U\otimes W)$.
:::
:::{.sol .callout-tip collapse="true"}
### Dica: 
...
aplicação $k$-linear $\lambda:V^k\to \Lambda^kV$ que satisfaz a seguinte propriedade universal: Para todo espaço vetorial $Z$ e toda
aplicação $k$-linear $f:V^k\to Z$, existe uma única aplicação linear $\tilde f:\Lambda^kV\to Z$ tal que 
...
1. **Diagrama da propriedade universal**:
   ...
   Aqui, a seta vertical $V^k \to \Lambda^kV$ é a aplicação $k$-linear $\lambda$, e a seta diagonal $\Lambda^kV \to Z$ é a aplicação linear $\tilde{f}$ que torna o diagrama comutativo, ou seja, $f = \tilde{f} \circ \lambda$.

2. **Unicidade de $\Lambda^kV$**:
   Seja $\Lambda^kV$ e $\Lambda'^kV$ dois espaços vetoriais que satisfazem a propriedade universal com as aplicações $k$-lineares $\lambda: V^k \to \Lambda^kV$ e $\lambda': V^k \to \Lambda'^kV$. Pela propriedade universal de $\Lambda^kV$, existe uma aplicação linear única $\phi: \Lambda^kV \to \Lambda'^kV$ tal que $\lambda' = \phi \circ \lambda$. Analogamente, pela propriedade universal de $\Lambda'^kV$, existe uma aplicação linear única $\psi: \Lambda'^kV \to \Lambda^kV$ tal que $\lambda = \psi \circ \lambda'$. 
...
1. **Propriedade multiplicativa do determinante**:
   - Seja $f, g: V \to V$. Pelo @def-determinant-f, o determinante de $f$ é definido como o escalar $\delta \in \F$ tal que $f^*_A = \delta \cdot \text{id}_{A^n(V)}$, onde $A^n(V)$ é o espaço das formas $n$-lineares alternadas em $V$. Similarmente, para $g$, temos $g^*_A = \gamma \cdot \text{id}_{A^n(V)}$.
   - Considere a composição $f \circ g$. O pullback $(f \circ g)^*_A$ é dado por:
     $$
     (f \circ g)^*_A = g^*_A \circ f^*_A.
     $$
     Substituindo as expressões para $f^*_A$ e $g^*_A$, temos:
     \begin{align*}
     (f \circ g)^*_A &= (\gamma \cdot \text{id}_{A^n(V)}) \circ (\delta \cdot \text{id}_{A^n(V)}) 
     \\&= (\gamma \cdot \delta) \cdot \text{id}_{A^n(V)}.
     \end{align*}
     Assim, $\det(f \circ g) = \det(f) \cdot \det(g)$.

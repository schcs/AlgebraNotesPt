...
3. **Construção do isomorfismo**:
   - Para cada série formal $f(x) = \sum_{k \geq 0} \alpha_k x^k \in \F[[x]]$, definimos um funcional linear $\varphi_f \in \F[x]^*$ como:
     $$
     \varphi_f\left(\sum_{j=0}^n \beta_j x^j\right) = \sum_{j=0}^n \beta_j \alpha_j.
     $$
     Em outras palavras, $\varphi_f$ avalia o polinômio $\sum_{j=0}^n \beta_j x^j$ nos coeficientes correspondentes da série formal $f(x)$.

4. **Verificação de linearidade**:
   - Seja $p(x), q(x) \in \F[x]$ e $\lambda \in \F$. Então:
     $$
     \varphi_f(\lambda p(x) + q(x)) = \sum_{j=0}^n (\lambda p_j + q_j) \alpha_j = \lambda \varphi_f(p(x)) + \varphi_f(q(x)).
     $$
     Portanto, $\varphi_f$ é linear.

5. **Injetividade**:
   - Se $\varphi_f = \varphi_g$, então para todo $p(x) \in \F[x]$, temos $\varphi_f(p(x)) = \varphi_g(p(x))$. Isso implica que $\alpha_k = \beta_k$ para todo $k \geq 0$, ou seja, $f(x) = g(x)$. Logo, o mapeamento é injetivo.

6. **Sobrejetividade**:
   - Seja $\varphi \in \F[x]^*$. Definimos uma série formal $f(x) = \sum_{k \geq 0} \alpha_k x^k$, onde $\alpha_k = \varphi(x^k)$. Então, para qualquer $p(x) = \sum_{j=0}^n \beta_j x^j \in \F[x]$, temos:
     $$
     \varphi(p(x)) = \sum_{j=0}^n \beta_j \varphi(x^j) = \sum_{j=0}^n \beta_j \alpha_j = \varphi_f(p(x)).
     $$
     Portanto, $\varphi = \varphi_f$, e o mapeamento é sobrejetivo.
...
$$
é um número par. [Dica: Use @exr-perm-par.]
:::

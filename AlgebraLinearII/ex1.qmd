...
\end{align*}
Ou seja, $U_1$ é o conjunto das matrizes simétricas, enquanto $U_2$ é o conjunto das matrizes antissimétricas em $V$.
...
1. **Definição do dual**:
   - O dual $f^\ast : W^\ast \to V^\ast$ é definido por $f^\ast(\varphi) = \varphi \circ f$ para todo $\varphi \in W^\ast$.
   - Isso significa que $f^\ast$ "puxa para trás" os funcionais lineares de $W^\ast$ para $V^\ast$.

2. **Representação matricial**:
   - Seja $[f]^B_C$ a matriz que representa $f$ em relação às bases $B$ e $C$.
   - Seja $[f^\ast]^{C^\ast}_{B^\ast}$ a matriz que representa $f^\ast$ em relação às bases duais $C^\ast$ e $B^\ast$.

3. **Propriedade das bases duais**:
    - Por definição, as bases duais $B^\ast = \{\beta_1^\ast, \ldots, \beta_n^\ast\}$ e $C^\ast = \{\gamma_1^\ast, \ldots, \gamma_m^\ast\}$ satisfazem:
     $$
       \beta_i^\ast(b_j) = \delta_{ij}, \quad \gamma_i^\ast(c_j) = \delta_{ij},
     $$
     onde $\delta_{ij}$ é o delta de Kronecker.

4. **Cálculo de $[f^\ast]^{C^\ast}_{B^\ast}$**:
    - Para calcular $[f^\ast]^{C^\ast}_{B^\ast}$, precisamos determinar as coordenadas de $f^\ast(\gamma_j^\ast)$ em relação à base $B^\ast$.
    - Por definição de $f^\ast$, temos:
     $$
       f^\ast(\gamma_j^\ast)(b_i) = \gamma_j^\ast(f(b_i)).
     $$
    - Isso significa que a entrada $(i, j)$ de $[f^\ast]^{C^\ast}_{B^\ast}$ é dada por $\gamma_j^\ast(f(b_i))$.

5. **Relação com $[f]^B_C$**:
   - A matriz $[f]^B_C$ é definida por $f(b_i) = \sum_{j=1}^m a_{ji} c_j$, onde $a_{ji}$ são as entradas de $[f]^B_C$.
   - Portanto, $\gamma_j^\ast(f(b_i)) = a_{ji}$.

6. **Conclusão**:
    - As entradas de $[f^\ast]^{C^\ast}_{B^\ast}$ são dadas por $a_{ji}$, que são as entradas transpostas de $[f]^B_C$.
   - Assim, temos:
     $$
       [f^\ast]^{C^\ast}_{B^\ast} = ([f]^B_C)^t.
     $$
...
4. **Mostre que $\mbox{Im}\,f^∗ = (\ker f)^0$**:
   ...
   de $\mbox{Im}(f)$ em $W$; ou seja $W=\mbox{Im}(f)\oplus Z$. Defina $\varphi$ em $\mbox{Im}(f)$ tal que $\psi(v) = \varphi(f(v))$ para todo $v \in V$ e defina $\varphi$ em $Z$ tal que $\varphi(z)=0$ para todo $z\in Z$. Note que $\varphi$ está bem definida e $\psi=f^*(\varphi)$. 

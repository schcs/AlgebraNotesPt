...
$$
Ou seja, $1^\sigma=-1$. Mas se $\mbox{car}(\F)\neq 2$, isso é impossível, pois $1^\sigma=1$ para todo automorfismo de $\F$.  Logo existe vetor $b_1\in V$ tal que $Q(b_1)=B(b_1,b_1)\neq 0$. Seja $U=\langle b_1\rangle$ e considere $U^\perp$. Temos que $\dim U^\perp=\dim V-1$ ($B$ é não degenerada). Além disso, $U\cap U^\perp=0$, pois se $w\in U\cap U^\perp$, então $u=\alpha b_1$ com algum $\alpa\in\F$ e $0=B(\alpha b_1,b_1)=\alpha Q(b_1)$ que implica que 

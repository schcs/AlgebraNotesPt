...
:::{#def-partition}
Se $n\in\N$, então uma *partição* de $n$ é um conjunto $\lambda=\{k_1,\ldots,k_m\}$ tal que  $k_1+\cdots+k_m=n$. Neste caso escrevemos que 
$\lambda\vdash n$. 
:::
...
:::{#exm-perm-part}
Cada permutação $\sigma\in S_n$ determina uma partição de $n$. De fato, assuma que $\sigma$ é produto de ciclos de comprimento $\lambda_1,\ldots,\lambda_m$ contando também os ciclos de comprimento $1$. Então $\lambda=\{\lambda_1,\ldots,\lambda_m\}\vdash n$. 
:::
...
:::{#exm-classes-s5}
Considere o grupo $S_5$. Pelo @exm-p5, este grupo tem 7 classes de conjugação com os seguintes representantes: $1$, $(1,2)$, $(1,2)(3,4)$, $(1,2,3)$, $(1,2,3)(4,5)$, $(1,2,3,4)$, e $(1,2,3,4,5)$. A seguinte tabela contém as cardinalidades destas classes, as cardinalidades do centralizador do representante, e geradores do centralizador.
...
O centro $Z(G)$ de um grupo $G$ foi definido no @def-center como
\begin{eqnarray*}
...
A cadeia construída na demonstração do @cor-solvable chama se *cadeia central superior* de $G$. O argumento na demonstração na verdade demonstra que um $p$-grupo finito $G$ é nilpotente que é uma propriedade mais forte que ser solúvel.
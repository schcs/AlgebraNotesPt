...
Note que a separabilidade de $f(x)$ e de $m_\alpha(x)$ no @def-sep-ext pode ser verificada usando o critério @exr-derivado.
...
$m_\beta(x)\in\F[x]$ sobre $\K$. Como $\E:\F$ é separável e o grau de $m_\beta(x)$ é maior que dois, existe $\beta'\in\L\setminus\{\beta\}$ tais que $m_\beta(\beta')=0$. Pelo @thm-transitive, existe $\sigma$ um automorfismo em $\gal{\L}{\K}$ tal que $\sigma(\beta)=\beta'$ e $\sigma(\alpha+c\beta)=\alpha+c\beta$. Portanto 
$$
...
$$
Como $\sigma\in \gal{\L}{\K}$, temos por @lem-gal-ff, temos um número finito de possibilidades para a expressão no lado direito da equação anterior. Isso quer dizer que temos apenas um número finito de possibilidades para $c\in\F$ tal que $\F(\alpha+c\beta)\neq \F(\alpha,\beta)$. Como $\F$ é infinito, existe $c\in\F$ tal que $\F(\alpha+c\beta)\neq \F(\alpha,\beta)$.
:::
...
:::
:::{.proof}
Seja $n=|\gal\E\F|$. Pelo  @lem-gal-ff, $n\leq\dim_\F\E$, então basta provar a outra desigualdade. Pelo @lem-simple-ext, temos que $\E=\F(\alpha)$ com algum $\alpha\in\E$. Afirmamos que $\mbox{grau}\,m_\alpha(x)\leq n$ e isso vai implicar que $\dim_\F\E\leq n$. Note que é suficiente achar um polinômio $g(x)\in\F[x]$ de grau $n$ tal que $\alpha$ é raiz de $g(x)$. Assuma que $\{\sigma_1,\ldots,\sigma_n\}=\gal\E\F$ com $\sigma_1=1$ e considere o polinômio
$$
g(x)=(x-\sigma_1(\alpha))\cdots(x-\sigma_n(\alpha))=x^n+\beta_{n-1}x^{n-1}+\cdots+\beta_1x+\beta_0.
$$
...
isomorfismo tal que $\psi|_\F=\mbox{id}_\F$ e $\psi(\alpha)=\psi(\beta)$. O corpo $\E$ é corpo de decomposição de algum polinômio $f(x)\in\F[x]$ sobre 
$\F$ e também sobre os corpos isomorfos $\F(\alpha)$ e $\F(\beta)$. Por @thm-decompfield-unique, existe um $\sigma\in\aut\E$ tal que $\sigma|_{\F(\alpha)}=\psi$. Em particular, $\sigma\in\gal\E\F$. Portanto, $\beta=\sigma(\K)=\K$, e temos que toda raiz de $p(x)$ está contida em $\K$. Logo $\K$ é extensão normal. 
:::
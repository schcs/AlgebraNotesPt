...
1. O que pode dizer sobre $\emptyset\times A$ and $A\times \emptyset$? Justifique.
...
5. $(A\times B)\cap (C\times D)=(A\cap B)\times (B\cap D)$. 

:::{#exr-}
Sejam $A$, $B$, $C$ e $D$ conjuntos.
Será que a igualdade $(A\times B)\cup (C\times D)=(A\cup B)\times (B\cup D)$ é 
...
:::{#exr-}
Um aluno gostaria de provar que se $R$ é uma relação transitiva e simétrica sobre um conjunto $A$, então $R$ é reflexiva. O aluno pediu fez o seguinte argumento: 
...
1. Será que a resposta do aluno está certa. Critique a resposta.

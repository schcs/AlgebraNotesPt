...
:::{#def-well-order}
Seja $X$ um conjunto com ordem total $\preceq$. Dizemos que $X$ está *bem ordenado* se todo subconjunto 
não vazio $Y\subseteq X$ possui menor elemento. Ou seja, se $Y\subseteq X$ tal que $Y\neq \emptyset$, 
temos que 
$$
\exists a\in Y:(\forall b\in Y: a\leq b).
$$
:::
...
:::{.proof}
Provaremos a seguinte afirmação por indução em $n$: *Se $Y\subseteq \N$ tal que $n\in Y$, então 
$Y$ possui menor elemento.* 

*Base da Indução:* Se $n=1$ e $n\in Y$, então $n=1$ é necessariamente menor elemento de $Y$, pois ele
é o menor natural por item 1. do @lem-order-nat. 

*Hipótese da Indução:* Assuma para todo $a\in\{1,\ldots,n\}=\{x\in \N\mid 1\leq x\leq n\}$ que se $a\in Y\subseteq \N$, então $Y$ possui menor elemento.

*Passo indutivo*: Assuma que $Y\subseteq \N$ tal que $S(n)\in Y$. Se $S(n)\leq y$ para todo 
$y\in Y$, então $S(n)$ é menor elemento de $Y$. Se existir $a\in Y$ tal que $a < S(n)$, então  
$a\in\{1,\ldots,n\}$ e tambem $a\in Y$ e a Hipótese da Indução implica que $Y$ possui menor elemento. 
:::

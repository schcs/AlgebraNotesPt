...
1. A *união* $A\cup B$ de $A$ é $B$ é o conjunto dos objetos que pertencem ao conjunto $A$ ou ao conjunto $B$ (incluindo a possibilidade que perntencem aos dois).
...
Sejam 
$$
A=\{1,2,3,5,6,7\}\quad \mbox{e}\quad A=\{2,3,4,6,8,9\}.
$$
Então temos que 
\begin{align*}
...
:::{#lem-set-operations}
Sejam $A$ e $B$ conjuntos. As seguintes afirmações são verdadeiras.

1. $A\subseteq A\cup B$, $B\subseteq A\cup B$;
2. $A\cap B\subseteq A$, $A\cap B\subseteq A$;
...
\end{align*}
Isso implica que $A\cap (B\cup C)\subseteq(A\cup B)\cap (A\cup C)$. 
...
A prova da afirmação 2. é similar e será exercício para o leitor.
:::
